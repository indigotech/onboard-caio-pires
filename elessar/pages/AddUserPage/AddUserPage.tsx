...
import { View, Keyboard, ScrollView, ActivityIndicator, StyleSheet, TouchableOpacity, Text, TextInput } from 'react-native';
import { validateEmail, validatePassword, validatePhone, validateName, validateRole} from '../Login/validation';
...
export default class AddUserPage extends Component<Props, AddUserPageState>{
    ...
    private handleButton = async () => {
        ...
        if(!isEmailValid){
            this.setState({emailMessage: 'E-mail inválido'});
        }
        else{
        ...
        if(!isPasswordValid){
            this.setState({passwordMessage: 'Senha deve ter ao menos 7 caracteres e 1 caracter numérico e 1 alfabético'});
        }
        else{
        ...
        if(!isNameValid){
            this.setState({nameMessage: 'Nome inválido'});
        }
        else{
        ...
        if(!isPhoneValid){
            this.setState({phoneMessage: 'Telefone inválido'});
        }
        else{
        ...
        if(!isRoleValid){
            this.setState({roleMessage: 'Função inválida. Deve ser \'admin\' ou \'user\''});
        }
        {
            this.setState({roleMessage: ''});
        }
        
        const allValid = isEmailValid && isPasswordValid && isNameValid && isPhoneValid && isRoleValid;
        ...
        return (
            ...
            <ScrollView style={styles.OuterView}>
                ...
                <View style={styles.field}>
                    ...
                    {this.state.showDatePicker && (
                        <DatePicker
                            ...
                            onClose={(d) => {this.setState({showDatePicker: false})}}
                            onChange={
                                (d) => {
                                    if(typeof d.nativeEvent?.timestamp != 'undefined'){
                                    this.setState({ birth: new Date(d.nativeEvent?.timestamp), showDatePicker: false});
                                    }}
                            }/>)}
                </View >
                <View style={styles.field}>
                ...
                <View style={styles.field}>
                    ...
                    />
                    <Text style={styles.msgText}>
                    {this.state.roleMessage}
                    </Text>
                    {/* <TouchableOpacity
                        onPress={() => this.setState({ showRolePicker: true})}>
                    </TouchableOpacity>
                    <Picker
                        label='Função'
